    ...
    def _step_muon(self, group):
        ...
        if "momentum_buffer" not in state:
            state["momentum_buffer"] = torch.zeros(
                num_params, *shape, dtype=dtype, device=dev
            )
        if "second_momentum_buffer" not in state:
            ...
            state["second_momentum_buffer"] = torch.zeros(
                state_shape, dtype=dtype, device=dev
            )
...
EMBEDDING_LR = 0.16  # learning rate for token embeddings (Adam)
UNEMBEDDING_LR = 0.16  # learning rate for lm_head (Adam)
MATRIX_LR = 0.04981411999566552  # learning rate for matrix parameters (Muon)
SCALAR_LR = 0.0012835619902141762  # learning rate for per-layer scalars (Adam)
WEIGHT_DECAY = 0.041  # cautious weight decay for Muon
...
train_loader = make_dataloader(
    tokenizer, DEVICE_BATCH_SIZE, MAX_SEQ_LEN, "train", device=device, buffer_size=100
)
