#!/usr/bin/env python3
"""
train_95m.py — GPT training with 95M parameter model for autoresearch.

Extends train.py with a 95M parameter configuration:
- n_embd=640, n_layer=8, n_head=20
- Optimized for CPU training with larger batch sizes

Usage:
    python train_95m.py                          # Default 300s budget
    python train_95m.py --time-budget 600        # 10-minute budget
    python train_95m.py --total-batch 65536      # Larger batch
"""
...
# Parse CLI flags BEFORE importing torch
parser = argparse.ArgumentParser(description="AutoResearch 95M training")
...
parser.add_argument("--lr-scale", type=float, default=1.0, help="Learning rate scale factor")
...
# ---------------------------------------------------------------------------
# 95M Model Configuration
# ---------------------------------------------------------------------------

HEAD_DIM = 32
WINDOW_PATTERN = _CLI_WINDOW if _CLI_WINDOW else "SSSL"

# 95M configuration: n_embd=640, n_layer=8, n_head=20
# Total params = 2 * vocab_size * n_embd + n_layer * 16 * n_embd^2
# = 2 * 32768 * 640 + 8 * 16 * 640^2 = 41.9M + 52.4M = 94.4M
N_EMBD_95M = 640
N_LAYER_95M = 8
N_HEAD_95M = N_EMBD_95M // HEAD_DIM  # 20 heads

# Batch sizes - optimized for CPU training
TOTAL_BATCH_SIZE = _CLI_TOTAL_BATCH if _CLI_TOTAL_BATCH else 2**14  # 16384 tokens
DEVICE_BATCH_SIZE = _CLI_BATCH_SIZE if _CLI_BATCH_SIZE else 8

# Learning rates - scaled for 95M model
EMBEDDING_LR = 0.0225 * _LR_SCALE
UNEMBEDDING_LR = 0.0225 * _LR_SCALE
MATRIX_LR = 0.037 * _LR_SCALE
SCALAR_LR = 0.038 * _LR_SCALE
WEIGHT_DECAY = 0.064
ADAM_BETAS = (0.8, 0.95)
WARMUP_RATIO = 0.0
WARMDOWN_RATIO = 0.5
...
print(f"Model: 95M config (n_embd={N_EMBD_95M}, n_layer={N_LAYER_95M}, n_head={N_HEAD_95M})")
print(f"DEVICE_BATCH={DEVICE_BATCH_SIZE} TOTAL_BATCH={TOTAL_BATCH_SIZE} WINDOW={WINDOW_PATTERN} TIME_BUDGET={TIME_BUDGET}s")
